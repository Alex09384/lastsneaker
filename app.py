...
def calculate_compatibility(user_data, shoe_size, is_sport=1):
    ...
    has_toe = user_data.get('toe_circumference') and str(user_data['toe_circumference']).strip()
    has_instep = user_data.get('instep_circumference') and str(user_data['instep_circumference']).strip()
    has_foot_type = user_data.get('foot_type') and str(user_data['foot_type']).strip()
    ...
    # Новый параметр: обхват у пальцев
    if has_toe and 'toeCircumference' in shoe_size:
        try:
            user_toe = float(user_data['toe_circumference']) * 10
            shoe_toe = shoe_size['toeCircumference']
            user_toe += 10 if is_sport == 1 else 15
            toe_diff = abs(user_toe - shoe_toe)
            if toe_diff <= 8:
                toe_score = 12
            elif toe_diff <= 15:
                toe_score = 9
                ...
                toe_score = 3
            compatibility += toe_score
            factors += 12
        except ValueError:
            pass

    # Новый параметр: подъём
    if has_instep and 'instepCircumference' in shoe_size:
        try:
            user_instep = float(user_data['instep_circumference']) * 10
            shoe_instep = shoe_size['instepCircumference']
            user_instep += 10 if is_sport == 1 else 15
            instep_diff = abs(user_instep - shoe_instep)
            if instep_diff <= 8:
                instep_score = 12
            elif instep_diff <= 15:
                instep_score = 9
    ...
    for shoe in shoes_db['sneakers']:
        ...
        for size in shoe['sizes']:
            compatibility = calculate_compatibility({
                ...
                'toe_circumference': user_data.get('toe_circumference'),
                'instep_circumference': user_data.get('instep_circumference'),
...
def shoe_detail(model_name):
    ...
    for size in shoe['sizes']:
        compatibility = calculate_compatibility({
            ...
            'toe_circumference': user_data.get('toe_circumference') if user_data else None,
            'instep_circumference': user_data.get('instep_circumference') if user_data else None,
...
def measure():
    ...
    if request.method == 'POST':
        ...
        toe_circumference = request.form.get('toe_circumference', '').strip()
        instep_circumference = request.form.get('instep_circumference', '').strip()
        ...
        # Проверяем новые поля (только что это числа)
        try:
            float(toe_circumference)
        except:
            errors.append("Некорректное значение обхвата у пальцев")

        try:
            float(instep_circumference)
        except:
            errors.append("Некорректное значение подъёма")
        ...
        # Сохраняем все измерения, включая новые
        save_user_measurements({
            'foot_length': length,
            'foot_width': width,
            'oblique_circumference': oblique_circumference,
            'toe_circumference': toe_circumference,
            'instep_circumference': instep_circumference,
            'foot_type': foot_type
        })
